...
近期已把单音短句、常用和弦、练习模板、首页快捷入口、首页热门推荐、学习页专题入口、曲谱展示字段和扩展曲目沉到 `packages/shared`。
后续可以继续把 Preview 内仍未共享的课程展示分组、推荐策略逐步沉淀。

## 推荐继续开发顺序

1. 调音器真机 development build 测试，按 `docs/TUNER_DEVICE_TEST.md` 记录结果。
2. 课程展示与推荐策略继续对齐：优先处理 Preview `courses` 与 shared `mvpCourseCatalog` 的字段差异。
