...
近期已把单音短句、常用和弦、练习模板、首页快捷入口、首页热门推荐和学习页专题入口沉到 `packages/shared`。
后续可以继续把 Preview 内仍未共享的曲谱展示、课程展示分组、推荐策略逐步沉淀。

## 推荐继续开发顺序

1. 调音器真机 development build 测试，按 `docs/TUNER_DEVICE_TEST.md` 记录结果。
2. 曲谱目录与歌曲展示数据对齐：优先处理 Preview `songs` 与 shared `beginnerSongCatalog` 的字段差异。
