...
近期已把单音短句、常用和弦、和弦库分类/收藏、练习模板、首页快捷入口、首页热门推荐、学习页专题入口、曲谱展示字段、扩展曲目、课程展示字段、推荐路径、练习模拟 fixture、节拍器速度预设和调音器展示配置沉到 `packages/shared`。
后续可以继续把 Preview 内仍未共享的细分展示常量逐步沉淀。

## 推荐继续开发顺序

1. 调音器真机 development build 测试，按 `docs/TUNER_DEVICE_TEST.md` 记录结果。
2. 继续整理 Preview 细分展示常量：优先检查曲谱详情的展示文案是否还能沉到 shared。
