...
近期已把单音短句、常用和弦、练习模板、首页快捷入口、首页热门推荐、学习页专题入口、曲谱展示字段、扩展曲目、课程展示字段和推荐路径沉到 `packages/shared`。
后续可以继续把 Preview 内仍未共享的练习模拟 fixture 逐步沉淀。

## 推荐继续开发顺序

1. 调音器真机 development build 测试，按 `docs/TUNER_DEVICE_TEST.md` 记录结果。
2. 练习模拟 fixture 继续对齐：优先处理 Preview `rhythmAutoOffsets`、`transitionAutoOffsets`、`followScorePattern` 等模拟评分数据。
